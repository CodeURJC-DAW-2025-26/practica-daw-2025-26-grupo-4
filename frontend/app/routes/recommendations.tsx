...
export default function Recommendations({ loaderData }: Route.ComponentProps) {
  ...
  const handleComboAddToCart = async (comboIds: string) => {
    const ids = comboIds.split(",");
    try {
      for (const id of ids) {
        if (id) await addItem(parseInt(id, 10), 1);
      }
    } catch (err: any) {
      if (err.message.includes("UNAUTHORIZED")) {
        navigate("/login");
        ...
        <div className="sidebar-column">
          <aside className="sidebar">
            ...
            <nav className="menu">
              ...
              <Link
                to="/recommendations"
                className="menu-item menu-item--rec menu-item--active"
                aria-current="true"
              >
          ...
          <div className="content-body">
                        ...
                        <div className="rec-combo-card" key={`combo-${idx}`}>
                          ...
                          <div className="rec-combo-products">
                            {rec.products.map((cp: any) => (
                              <Link
                                to={`/product/${cp.id}`}
                                className="rec-combo-item"
                                key={cp.id}
                              >
                                <div className="rec-combo-img-wrap">
                                  <img
                                    src={cp.mainImage || cp.images?.[0]?.url}
                                    alt={cp.name}
                                  />
                                </div>
                                <div className="rec-combo-info">
                                  <span className="rec-combo-name">
                                    {cp.name}
                                  </span>
                                  <span className="rec-combo-price">
                                    €{cp.formattedPrice || cp.price?.toFixed(2)}
                                  </span>
                                </div>
                              </Link>
                            ))}
                          ...
                          <div className="rec-combo-footer">
                            <span className="rec-combo-total">
                              Total del combo:{" "}
                              <strong>€{rec.totalPrice}</strong>
                            </span>
                            <button
                              className="btn btn--primary btn--sm"
                              onClick={() =>
                                handleComboAddToCart(
                                  rec.products.map((p: any) => p.id).join(","),
                                )
                              }
                            >
                        ...
                        >
                          <Link to={`/product/${sp.id}`}>
                            ...
                            <div className="card-details">
                              ...
                              <div className="price">
                                €{sp.formattedPrice || sp.price?.toFixed(2)}
                              </div>
            ...
            {(!recommendationsData.recommendations ||
              ...
              <p className="rec-empty">
                ...
                Aún no tenemos recomendaciones para ti. ¡Realiza tu primera
                compra!
              </p>
            )}
          </div>
